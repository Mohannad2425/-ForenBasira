...
def check_content(api_key, message):
    if len(message) == 0:
        return None
    else:
        with gemini_lock:
            ...
            prompt = f'''
You are a senior cybersecurity analyst. Here is a list of observed Windows Security Event IDs from the newest to oldest:
{message}
